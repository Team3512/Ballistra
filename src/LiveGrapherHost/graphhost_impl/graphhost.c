...
int sockets_listen_int(int port, sa_family_t sin_family, uint32_t s_addr) {
    ...
    error = bind(sd, (struct sockaddr*)&serv_addr, sizeof(struct sockaddr_in));
    if (error != 0) {
        perror("");
        close(sd);
        return -1;
...
struct graphhost_t* GraphHost_create(int port) {
    /* int i; */
...
/* Create a pipe for IPC with the thread */
    ...
    /* Launch the thread */
    if (pthread_create(&inst->thread, NULL, sockets_threadmain, (void*)inst) !=
        0) {
...
void GraphHost_destroy(struct graphhost_t* inst) {
...
void sockets_listremovesocket(struct list_t* list, struct socketconn_t* conn) {
...
void sockets_close(struct list_t* list, struct list_elem_t* elem) {
...
void sockets_remove_orphan(struct socketconn_t* conn) {
    ...
    for (dataset = conn->datasets->start; dataset != NULL;
...
void sockets_clear_orphans(struct list_t* list) {
...
int sockets_readdoneh(struct graphhost_t* inst, uint8_t* inbuf, size_t bufsize,
                      struct list_t* list, struct list_elem_t* elem) {
    struct socketconn_t* conn = elem->data;
    char* buf;
    char* graphstr;
    char* tmpstr;
...
int sockets_sendlist(struct graphhost_t* inst, struct list_t* list,
                     struct list_elem_t* elem) {
    ...
    for (clelem = graphlist->start; clelem != NULL; clelem = clelem->next) {
        tmpstr = clelem->data;
        ...
        if (sockets_queuewrite(inst, conn, (void*)&replydg,
...
int sockets_writeh(struct graphhost_t* inst, struct list_t* list,
                   struct list_elem_t* elem) {
    int error;
    struct socketconn_t* conn = elem->data;
    ...
    while (1) {
        ...
        conn->writebufoffset +=
            send(conn->fd, (char*)conn->writebuf,
 ...
 */
int sockets_queuewrite(struct graphhost_t* inst, struct socketconn_t* conn,
                       uint8_t* buf, size_t buflength) {
    int error;
    struct writebuf_t* writebuf;
...
int socket_recordgraph(struct list_t* graphlist, const char* dataset) {
    char* graphstr;
    int graphinlist;
