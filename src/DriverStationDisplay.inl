...
template <class T>
DriverStationDisplay<T>* DriverStationDisplay<T>::getInstance(uint16_t dsPort) {
    if (m_dsDisplay == NULL) {
...
void DriverStationDisplay<T>::sendToDS(sf::Packet* userData) {
    ...
    if (userData == NULL) {
        SocketInit::getInstance().send(*static_cast<sf::Packet*>(this), testIP,
                                       m_dsPort);
    } else {
...
template <class T>
DriverStationDisplay<T>::DriverStationDisplay(uint16_t portNumber)
...
namespace DS {
template <class T>
inline void AddElementData(DriverStationDisplay<T>* inst, std::string ID,
                           DS::StatusLight data) {
