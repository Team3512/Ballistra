...
Claw::Claw(uint32_t clawRotatePort, uint32_t clawWheelPort,
...
Claw::~Claw() {
    // Free solenoids
    for (uint32_t i = 0; i < m_ballShooter.size(); i++) {
...
double Claw::GetTargetAngle() const { return m_clawRotator->getSetpoint(); }

float Claw::GetWheelManual() const { return m_clawRotator->getManual(); }

double Claw::GetAngle() { return m_clawRotator->getDistance(); }

bool Claw::AtAngle() const { return m_clawRotator->onTarget(); }

void Claw::SetWheelSetpoint(float speed) { m_intakeWheel->setSetpoint(speed); }

void Claw::SetWheelManual(float speed) { m_intakeWheel->setManual(speed); }
...
bool Claw::GetCollectorMode() { return m_collectorArm->Get(); }
...
void Claw::Update() {
    ...
    if (std::fabs(m_clawRotator->getRate()) > 35.f) {
        SetWheelManual(-1.f);
    }
    ...
    DriverStationLCD::GetInstance()->PrintfLine(DriverStationLCD::kUser_Line1,
                                                "Angle: %f",
                                                m_clawRotator->getDistance());
    DriverStationLCD::GetInstance()->PrintfLine(
        DriverStationLCD::kUser_Line2, "A Setpt: %f", GetTargetAngle());
    DriverStationLCD::GetInstance()->PrintfLine(
        DriverStationLCD::kUser_Line3, "Limit On: %u", !m_zeroSwitch->Get());
    DriverStationLCD::GetInstance()->PrintfLine(
        DriverStationLCD::kUser_Line4, "OnTarget: %u",
        static_cast<uint32_t>(m_clawRotator->onTarget()));
    DriverStationLCD::GetInstance()->PrintfLine(
        DriverStationLCD::kUser_Line5, "A Rate: %f", m_clawRotator->getRate());
    DriverStationLCD::GetInstance()->UpdateLCD();

    std::cout << "error: " << m_clawRotator->m_pid->GetError()
              << " tolerance: " << m_clawRotator->m_pid->GetTolerance()
              << std::endl;
}

void Claw::setF(float f) { m_clawRotator->setF(f); }

void Claw::setK(float k) { m_k = k; }
...
float Claw::calcF() {
    ...
    return m_k * std::cos((GetAngle() + m_l) * M_PI / 180.0f) /
           GetTargetAngle();
}
bool Claw::onTarget() { return m_clawRotator->onTarget(); }
...
void Claw::ResetClawEncoder(uint32_t interruptAssertedMask, void* obj) {
...
void Claw::CloseClaw(uint32_t interruptAssertedMask, void* obj) {
