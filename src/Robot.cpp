...
Robot::Robot()
    ...
    driverStation->addAutonMethod("DriveForward Autonomous",
                                  &Robot::DriveForwardAuton, this);
    driverStation->addAutonMethod("Right/Left Autonomous",
                                  &Robot::RightLeftAuton, this);
    driverStation->addAutonMethod("MotionProfile", &Robot::AutonMotionProfile,
                                  this);
...
void Robot::OperatorControl() {
    ...
    while (IsOperatorControl() && IsEnabled()) {
        ...
        // Shoots Ball
        if ((shootStick->GetRawButton(1) || driveStick2->GetRawButton(1)) &&
            !claw->IsShooting()) {
            claw->Shoot();
        }
...
bool Robot::testDriveTrain(bool shifterState, bool direction, float lowerBound,
                           float upperBound) {
    Timer timer;
    timer.Start();
...
void Robot::DS_PrintOut() {
    ...
    if (displayTimer->HasPeriodPassed(0.5)) {
        // (*ls) << SetLogLevel(LogEvent::VERBOSE_INFO) <<
        // kinect->GetArmScale().second << std::flush;
        // logServerSink->acceptor(false);

        // DriverStationLCD *userMessages = DriverStationLCD::GetInstance();
        // userMessages->Clear();

        // userMessages->Printf(DriverStationLCD::kUser_Line1, 1,"accelerometer
        // %f ",accelerometer->GetAcceleration(ADXL345_I2C_ALT::kAxis_X));

        // userMessages->Printf(DriverStationLCD::kUser_Line2, 1,"Encoder2:
        // %f",robotPosition->GetLeftEncoder());
        // userMessages->Printf(DriverStationLCD::kUser_Line3, 1," Left : %f",
        // (driveStick1->GetTwist() + 1)/2);
        // userMessages->Printf(DriverStationLCD::kUser_Line4, 1," Right : %f",
        // (driveStick2->GetTwist() + 1)/2 );

        // userMessages->UpdateLCD();

        driverStation->clear();

        DS::AddElementData(driverStation, "LEFT_RPM",
                           robotDrive->getLeftRate());
        DS::AddElementData(driverStation, "RIGHT_RPM",
                           robotDrive->getRightRate());
        DS::AddElementData(driverStation, "LEFT_DIST",
                           robotDrive->getLeftDist());
        DS::AddElementData(driverStation, "RIGHT_DIST",
                           robotDrive->getRightDist());

        DS::AddElementData(driverStation, "ANGLE_SET_DISP",
                           claw->GetTargetAngle());
        DS::AddElementData(
            driverStation, "ANGLE_SET",
            static_cast<int8_t>(claw->GetTargetAngle() / 175.f * 100.f));

        DS::AddElementData(driverStation, "ANGLE_REAL_DISP", claw->GetAngle());
        DS::AddElementData(
            driverStation, "ANGLE_REAL",
            static_cast<int8_t>(claw->GetAngle() / 175.f * 100.f));
