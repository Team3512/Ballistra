...
double TrapezoidProfile::updateSetpoint(double curSetpoint, double curSource,
                                        double curTime) {
...
double TrapezoidProfile::setGoal(double goal, double curSource, double t) {
    ...
    double deltaPosMaxV =
        (m_sign * setpoint) - (m_timeToMaxVelocity * m_velocity);
...
bool TrapezoidProfile::atGoal() { return m_lastTime > m_timeTotal; }
...
void TrapezoidProfile::resetTime() { m_lastTime = 0.0; }

void TrapezoidProfile::setMode(SetpointMode mode) { m_mode = mode; }

SetpointMode TrapezoidProfile::getMode() { return m_mode; }
