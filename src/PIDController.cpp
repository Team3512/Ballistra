...
float PIDController::GetTolerance() { return m_tolerance; }
...
void PIDController::CallCalculate(void* controller) {
    auto control = static_cast<PIDController*>(controller);
    control->Calculate();
}
...
float PIDController::GetError() {
    float error;
    CRITICAL_REGION(m_semaphore) { error = m_setpoint - m_pidInput->PIDGet(); }
    END_REGION;
    return error;
}
...
bool PIDController::OnTarget() {
    ...
    CRITICAL_REGION(m_semaphore) {
        switch (m_toleranceType) {
            case kPercentTolerance:
                temp = std::fabs(GetError()) <
                       (m_tolerance / 100 * (m_maximumInput - m_minimumInput));
                break;
            case kAbsoluteTolerance:
                temp = std::fabs(GetError()) < m_tolerance;
                break;
    ...
    /* Return as on target if error is in acceptable bounds and change in error
...
float PIDController::GetDeltaError() { return m_deltaError; }
...
/**
 * Reset the previous error and the integral term, and disable the controller.
 */
